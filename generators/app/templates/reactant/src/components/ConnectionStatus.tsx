...
import { Card, Spin, Alert, Descriptions, Tag, Row, Col, Space, Button } from 'antd';
import { LoadingOutlined, CheckCircleOutlined, WarningOutlined, CopyOutlined, BgColorsOutlined, SaveOutlined } from '@ant-design/icons';
...
export const ConnectionStatus: React.FC<ConnectionStatusProps> = ({ connection, isLoading, onLog }) => {
    ...
    if (isLoading) {
        return (
            <Card title="🔗 Connection Status" style={{ marginBottom: 24 }}>
                <Spin indicator={<LoadingOutlined style={{ fontSize: 24 }} spin />} tip="Checking connection...">
    ...
    return (
        <Card title="🔗 Connection Status & Utilities" style={{ marginBottom: 24 }}>
            <Row gutter={[16, 16]}>
                <Col xs={24} lg={16}>
                    <Alert message="Connected" type="success" icon={<CheckCircleOutlined />} showIcon style={{ marginBottom: 16 }} />
                    <Descriptions bordered column={1}>
                        <Descriptions.Item label="Name">{connection.name}</Descriptions.Item>
                        <Descriptions.Item label="URL">{connection.url}</Descriptions.Item>
                        <Descriptions.Item label="Environment">
                            <Tag color={getEnvTagColor(connection.environment)}>{connection.environment.toUpperCase()}</Tag>
                        </Descriptions.Item>
                ...
                <Col xs={24} lg={8}>
                    <Card title="🔧 Utilities" size="small" style={{ height: '100%' }}>
                        <Space direction="vertical" style={{ width: '100%' }}>
                            ...
                            <Button icon={<BgColorsOutlined />} onClick={showCurrentTheme} block>
                                Get Theme
                            </Button>
